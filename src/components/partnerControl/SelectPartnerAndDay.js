...
class SelectPartnerAndDay extends Component {
    ...
    CDU_setStateListDay = () => {
        if (this.props.items.type === "getListDayById") { // nếu type = getListDayById
            ...
            if (this.state.listDay.join("") !== listDay.join("")) { // khi listDay thay đổi thì mới setState (listDay) mới
                ...
                if (this.props.date.from === undefined && this.props.date.to === undefined) {  // khi chưa  select Date thì GET (getListByCustom) với endpoint với 7 ngày gần nhaatys ứng với (partnerSelect)
                    ...
                    let endPoint = this.getEndPoint(this.state.partnerSelect, dateFrom, dateTo); // gọi hàm tạo endpoint ứng với date đã select
    ...
    getEndPoint = (partnerSelect, dateFrom, dateTo) => {  // hàm để tạo endpoint
    ...
    render() {
        ...
        if (listPartner !== null) { // chuyển listPartner=[] thành các DOM <p/> để render ra màn hình
            ...
            renderListPartner = listPartner.map((param, id) => {
                return <p className={"renderListPartner" + ((this.state.partnerSelect === param) ? " renderListPartner_select" : "")}
                    key={id} onClick={() => this.callAPIAndSetPartnerSelect(param)}>{param}</p>
            })
        }
        return (
            <React.Fragment>
                {renderListPartner} {/* render các DOM </p> ra màn hình*/}
                <SelectDate partnerSelect={this.state.partnerSelect} {...this.props} /> {/* component để select Date*/}
            </React.Fragment>
        );
    }
}
